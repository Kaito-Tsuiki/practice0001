...
//チェックボックス操作時処理
function incellCheckFunc2(checkIndex, checkRowData){
    //行のID
    let rowId = document.getElementById("checkboxInCell2_" + checkIndex).parentElement.parentElement.parentElement;

    //チェックオンされたとき配列に行要素を追加し行削除ボタンを活性化
    if(document.getElementById("checkboxInCell2_" + checkIndex).checked){
        let gridData = $(gridId2).datagrid('getRows');
        //最終行(新規データ行)がある場合とそうでない場合とで行数を調整
        let dataLength = (newRowFlg) ? gridData.length-1 : gridData.length;
        let lastRowCheck = (newRowFlg) ? JSON.stringify(gridData[dataLength]) !== JSON.stringify(checkRowData) : true
        //最終行(新規データ行)は削除を行わないためチェックする
        if(lastRowCheck){ 
            for(let i=0; i<dataLength; i++){
                if(JSON.stringify(checkRowData) === JSON.stringify(gridData[i])){
                    //削除行位置の取得
                    checkRows2.push(gridData[i]);
                    $('#deleteRowButton2').linkbutton('enable');
                    break;
                }
            }
            ...
            document.getElementById("checkboxInCell2_" + checkIndex).checked = false;
        }
    }
    //チェックオフされた場合配列から要素を削除し、配列の要素がなければ行削除ボタンを非活性化
    else{
        checkRows2 = checkRows2.filter(function(checkRow){
            return JSON.stringify(checkRow) !==  JSON.stringify(checkRowData);
        });

        if(checkRows2.length < 1){
            $('#deleteRowButton2').linkbutton('disable');
        }
